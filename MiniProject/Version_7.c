...
/*
realloc
create  initial array on  stack in main then fill it and pass address to the book ptr after filling
*/
...
//	size=n;
	...
	//Book M_Book[size];//we cannot do this also bcz realloc is extending that memory if not memory available then it change the object so here array is on the stack
...
if(index!=(-1) && index!=(-2)){
	...
	for(int i=index;i<(size-1);i++)// n-1  --> to avoid access of the garbage value and shifting
	{
		...
		size=size-1;////update the current size
