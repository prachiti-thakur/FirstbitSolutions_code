	 		...
	 		case 1:
	 			{
	 				...
	 				//free(book);
	 				book=newbook;//update book value with updated ptr
...
Book* addBook(Book* book){
	...
	//not neccessary to check size here with updated size bcz we are updating size here first as per user requirements and then filling
		...
		Book* reBook=(Book*)realloc(book,sizeof(Book)*(size+num));
		for(int i=n;i<(size+num);i++){
...
//				scanf("%d",&book[i].bookId);
				...
				gets(reBook[i].bookName);
				
				fflush(stdin);
				printf("Enter the Author:");
				gets(reBook[i].author);
				...
				scanf("%d",&cat);
				if(cat==1){
					strcpy(reBook[i].category,"Fictional");
				}
				else{
					if(cat==2){
						strcpy(reBook[i].category,"Non-Fictional");
						}
					else{
						...
						printf("Category:\n\t1.Fictional\n\t2.Non.Fictional\n");
								...
								if(cat==1){
									...
									if(cat==2){
										...
										strcpy(reBook[i].category,"Invalid choice");
										}
									}
					}
				}
				...
				printf("Enter price:");
				scanf("%lf",&reBook[i].price);
				
				float rate;
				printf("Enter Rating:");
				scanf("%f",&rate);
				if(rate<=5.0)
					reBook[i].rating=rate;
				else{
