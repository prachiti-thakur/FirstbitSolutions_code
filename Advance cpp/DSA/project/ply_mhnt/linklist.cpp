#include"linklist.h"
#include<fstream>
//1st write in file -->destructor
//2nd read from file ->constructor
template<class T>
LinkedList<T>::LinkedList(){
	this->start=NULL;
	ifstream in("Player.bin",ios_base::binary);
	T obj;
	while(!in.eof()){
...
LinkedList<T>::~LinkedList(){
	...
	ofstream out("Player.bin",ios_base::binary);
	
	while(start!=NULL){
		p=this->start;
		out.write((char*)&(p->getPly()),sizeof(p->getPly()));
		start=p->getNext();
		delete p;
	}
}
